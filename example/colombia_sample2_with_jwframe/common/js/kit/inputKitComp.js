...
define(['util'], function(){
	return {
		check: function(answerArr, userAnswerArr, options){
			options = options || {};
			this.options = {
				//초기입력상태 한/영, true면 한국어 기본
				ime: 					false,
				
				//대소문자를 판단하지 않을 답의 인덱스 번호를 적음
				//ex) "0,1" 
				freeCaseJudge: 				"",
				...
				//ex) ["0,1", "2,3,4"] // 0,1 그리고 2,3,4는 답을 바꿔적어도 정답 판단이 됨.
				freeOrderJudge: 		[],//
				
				//그룹단위의 입력순서를 판단하지 않을 답의 인덱스 번호를 적음
				//ex) [["0,1", "2,3"], ["4,5", "6,7"]]   // [0,1]과 [2,3] 교환가능  [4,5]와 [6,7] 교환가능
				//ex) [["0,1,2", "4,5,6"]]               // [0,1,2]와 [3,4,5] 교환가능
				freeGroupOrderJudge: 	[[]]
